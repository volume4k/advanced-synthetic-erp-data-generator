...
from erp_trace_executor.tools.fiori.create_supplier_invoice import (
    SUPPLIER_INVOICE_FORM_TIMEOUT_MS,
    SapSupplierInvoiceFlow,
)


class FakeSupplierInvoicePage:
    def __init__(self, *, draft_visible: bool) -> None:
    ...
    def get_by_role(self, role: str, *, name: str):
        return FakeSupplierInvoiceLocator(self, f"role:{role}:{name}", visible=True)
...
def test_supplier_invoice_draft_dialog_waits_for_slow_app_load_and_dismisses():
    page = FakeSupplierInvoicePage(draft_visible=True)

    SapSupplierInvoiceFlow(page)._discard_existing_draft_if_present(page)

    assert ("text:Rechnungsentwurf vorhanden", "visible", SUPPLIER_INVOICE_FORM_TIMEOUT_MS) in page.waits
    assert page.clicks == ["role:button:Nein"]
    assert ("role:textbox:Rechnungsdatum", "visible", SUPPLIER_INVOICE_FORM_TIMEOUT_MS) in page.waits


def test_supplier_invoice_draft_dialog_absence_keeps_existing_flow():
    page = FakeSupplierInvoicePage(draft_visible=False)

    SapSupplierInvoiceFlow(page)._discard_existing_draft_if_present(page)

    assert page.clicks == []
    assert ("role:textbox:Rechnungsdatum", "visible", SUPPLIER_INVOICE_FORM_TIMEOUT_MS) not in page.waits
